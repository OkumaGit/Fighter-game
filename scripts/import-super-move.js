...
const LOCAL_PATH = path.resolve(
  "client/resources/fighters/fighter-game-assets/Moves 11/Upscaled/sprite_super_moves_upscayl_2x_digital-art-4x.png",
);
...
async function processSuperMove() {
  ...
  console.log(
    `  ✓ Built 6560x820 WebP strip (${(webpBuffer.length / 1024).toFixed(1)} KB)`,
  );

  for (const id of FIGHTER_IDS) {
    const fighterResourcesDir = path.join(
      RESOURCES_BASE_DIR,
      `fighter_${id}_sprite`,
    );
